...
> **Motivo:** Documenta `ai-providers.ts` com a cadeia de fallback Groq/Cerebras/Mistral — o arquivo foi renomeado para `scoring.ts` na Fase A depois que a pilha de provedores inteira (código inalcançável; `callAI` sempre delegava para `scoreWithoutAI`) foi apagada.
...
**Prerequisite:** Run `docs/migracoes/10_party_positions.sql` in Supabase SQL Editor, then run `npm run ingest-party-programs` from `scripts/`.
