...
> **Motivo:** Prompt de sistema para enviar dados de candidato a um LLM de terceiros (Groq, etc.) — o pipeline que o consumia foi removido na Fase A. A pesquisa de candidato hoje é feita por um agente Claude Code seguindo `docs/procedimentos/pesquisa-de-candidato.md`, não por chamada de API a um provedor externo.
> **Substituído por:** `docs/procedimentos/pesquisa-de-candidato.md`
