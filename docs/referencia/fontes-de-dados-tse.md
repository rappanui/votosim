...
**Context:** The canonical way to obtain official 2026 electoral data, and the traps that break naive attempts. Read before writing any script that downloads from the TSE or parses its CSVs. Supersedes the source guidance in `docs/legado/06_data_pipeline.md`, which was written against 2022 data. All findings verified on 2026-08-20.
...
`presidente` and `vice_presidente` carry `SG_UF = 'BR'`. The `brazilian_state` enum in `docs/legado/base/01_schema_politicians.md` originally had no such value, so a schema rebuilt from the docs rejected every presidential candidacy with `invalid input value for enum brazilian_state: "BR"`. The live database had accepted `BR` all along — docs and database had silently diverged. Fixed in `docs/migracoes/11_sp0_foundation.sql` with an idempotent `ALTER TYPE`.
