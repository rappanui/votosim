...
**Context:** Practical guide for the three scripts that populate `politician_positions` for senators and deputies — offices that have no government plan PDFs. Read before running `ingest-party-programs`, `ingest-camara-votes`, or `ingest-senado-votes`. For why these scripts exist, see `docs/13_legislative_votes.md`. For the existing executive-branch scripts, see `docs/11_pipeline_scripts.md`.
