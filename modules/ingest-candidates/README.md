...
# Rode a partir da raiz deste pacote. Dois detalhes que parecem gratuitos e não são:
# o import precisa ser absoluto ($PWD/...) porque um script -e não resolve caminho
# relativo, e o argumento chega em argv[1], não em argv[2], porque sob -e não existe
# caminho de script ocupando essa posição.
npx tsx -e "
import { validateResearch } from '$PWD/src/lib/research-contract.ts'
...
`FINALIZADO` que vocês pediram) e não precisa de nenhum passo manual extra
...
marca a etapa `documentos_oficiais` como `nao_aplicavel` sozinho, e por isso
...
código inexistente devolve o mesmo total, então não confunda apresentados com
...
antes de inserir, de modo que rodar os dois em qualquer ordem destrói o trabalho do
...
- Um candidato reivindicado continua aparecendo em `next-candidates` (por
  design, porque se o processo travar no meio ele não pode sumir da fila como se
...
4. **Sem evidência é `neutro`, nunca é inventado.** `neutro` exige
   ...
   pesquisado e não achado. `fonteRefs` nunca fica vazio, e mesmo numa posição
...
5. **Plano de governo ausente ou ilegível é dito explicitamente**, nunca
   ...
   estar no snapshot local do TSE, então confira a imprensa antes de concluir que
