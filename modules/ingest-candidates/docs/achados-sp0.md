...
> **Exceção conhecida ao teto de 200 linhas de `docs/referencia/`** do
> repositório principal: este é um log append-only; dividi-lo por tamanho
> destruiria a cronologia dos achados. Ver `docs/referencia/README.md` no
> repositório principal (não incluído neste pacote). Esta é a cópia deste
> log que viaja no pacote de colaboração externa
> (`modules/ingest-candidates/`).
>
> **Origem:** cópia de `docs/registros/achados-sp0.md` do repositório
> principal.
...
disso, mesma postura da regra de `docs/pesquisa-de-candidato.md`
...
automaticamente (a regra de auto-validação é estendida para ele) e renderiza
com o selo `amarelo`. Ver `docs/referencia/schema-adicoes-sp0.md` no
repositório principal (não incluído neste pacote).
...
`docs/pesquisa-de-candidato.md` ou do contrato de ingestão. Duas
