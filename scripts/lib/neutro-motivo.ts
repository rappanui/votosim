 ...
 * ("não foi encontrada", "não retornaram", "não menciona"), whereas a stance
 * description negates the object ("não a simplificação").
 */
const ABSENCE_PATTERNS: RegExp[] = [
  /n[ãa]o (foi|foram)\s+(encontrad|localizad|identificad)/i,
  /n[ãa]o (retorn|apresent|menciona|aborda|trata|consta|cont[ée]m|possui|traz|registra)/i,
  /n[ãa]o (h[áa]|existe|se encontra)\s/i,
  /n[ãa]o (faz|apresenta|traz)\s+(nenhum|qualquer|men[çc][ãa]o|refer)/i,
